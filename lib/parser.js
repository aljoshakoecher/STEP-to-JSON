...
class StepToJsonParser {
    ...
    preprocessFile() {
        ...
        try {
            ...
            throw new Error(`Error while reading the file, filePath: ${this.filePath}`, error);
        }
    ...
    writeFile(content) {
        const filePathElems = this.filePath.split('\\');
        const inputFileWithEnding = filePathElems[filePathElems.length - 1];
        const inputFileWithoutEnding = inputFileWithEnding.split(".")[0];
        const outputFilePath = `.\\${inputFileWithoutEnding}.json`
